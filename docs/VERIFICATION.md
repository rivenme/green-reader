...
Firefox's CI trace showed long frame-related waits while ordinary DOM reads remained responsive. Neither software vsync nor a visible browser window resolved this on the Apple-based macOS 26 runner. Those launch overrides were removed. Firefox now uses the standard `macos-15-intel` runner; Chrome and Safari use `macos-26`. Mozilla documents [graphics failures with Apple virtual GPUs](https://bugzilla.mozilla.org/show_bug.cgi?id=2028438) and [related macOS VM stalls with different behavior on Intel](https://bugzilla.mozilla.org/show_bug.cgi?id=2010507); these support investigating the host, but do not prove the exact cause of this app's CI failure. Both architectures are [standard GitHub runners](https://docs.github.com/en/actions/reference/runners/github-hosted-runners).
