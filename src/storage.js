...
import {freshDrillStats} from './drills.js';
...
export const defaultSettings=()=>({stimp:10,optSlope:true,optPath:false,optBest:false,optGrid:true,optGrain:true,optErr:false,optSnd:true,optMotion:false,optHaptics:true,optPace:true,optCompare:false,controlMode:'stroke',strokeScale:'standard',cupMode:'forgiving',drillSlope:'flat',quality:'auto',tutorialDone:false});
...
export function validateSettings(raw){
  ...
  for(const [key,values] of Object.entries({controlMode:['stroke','distance','adaptive'],strokeScale:['standard','precision','long'],cupMode:['forgiving','realistic'],drillSlope:['flat','gentle','mixed']})){
    if(values.includes(raw[key]))s[key]=raw[key];
  }
  return s;
}
export function validateDrillStats(raw){
  const result=freshDrillStats();if(!record(raw))return result;
  for(const type of Object.keys(result)){
    const r=raw[type];
    if(record(r)&&count(r.attempts)&&count(r.hits)&&r.hits<=r.attempts&&count(r.bestStreak)&&r.bestStreak<=r.hits&&finite(r.totalLeave,0,1e12))result[type]={attempts:r.attempts,hits:r.hits,totalLeave:r.totalLeave,bestStreak:r.bestStreak};
  }
  return result;
}
...
export function validateRun(r){
  ...
  if(r.firstLeave!=null && !finite(r.firstLeave,0,100))return null;
...
export function createStore(storage, onIssue=()=>{}){
  ...
  const state={version:1,career:validateCareer(valid?raw.career:get('gr3d-career')),settings:validateSettings(valid?raw.settings:null),stats:validateStats(valid?raw.stats:get('gr3d-stats')),drills:validateDrillStats(valid?raw.drills:null),run:valid?validateRun(raw.run):null};
  if(valid && raw.run && !state.run)report(raw.run.courseVersion<COURSE_VERSION?'The greens have been updated. Start a fresh round; your XP and settings are saved.':'The saved run was invalid. Your valid XP and settings were recovered.');
